...
// Events
AsyncTest.waitOn('object_saved');

// Test
function test(tryNum) {
    ...
    query.first({
        success: function(object) {
            tryNum++;
            if (object) {
                AsyncTest.triggerEvent('object_saved', true);     
            } else if (tryNum < 3) {
                setTimeout(setTimeout(function(){test(tryNum);}, 1000));
            } else {
                AsyncTest.triggerEvent('object_saved', false, 'Looks like you didn\'t save your object');
            }
        },
...
$('.submit_button').click(function(e) {
    if (Parse.User.current()) {
        $(".loading").show();
        // Will test for 10 seconds then give up
        test(0);
    } else {
        AsyncTest.fail();
    }
